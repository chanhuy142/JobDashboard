...
# Các ngành nghề
job_titles = (
    set(df["Công việc chính"].unique())
    .union(set(df["Công việc liên quan 1"].unique()))
    .union(set(df["Công việc liên quan 2"].unique()))
)

job_titles = ["Tất cả"] + sorted(
    [title for title in job_titles if pd.notna(title)]
)
...
with st.sidebar:
    ...
    if gender != "Tất cả":
        df_filter = df_filter[
            df_filter["Yêu cầu giới tính"].str.contains(gender)
        ]
...
with col2:
    ...
    fig = px.box(
        ...
        x="Yêu cầu giới tính",
        y="Lương trung bình",
        color="Yêu cầu giới tính",
        color_discrete_map={
            "Nam": "skyblue",
            "Nữ": "salmon",
            "Không yêu cầu": "lightgreen",
        },
    )
    st.plotly_chart(fig, use_container_width=True)
...
with col3:
    ...
    fig = px.bar(
        df_job_level, x="Cấp bậc", y="Số lượng công việc", color="Cấp bậc"
    )
    st.plotly_chart(fig, use_container_width=True)
...
with col4:
    ...
    fig = px.pie(
        df_job_type,
        values="Số lượng công việc",
        names="Hình thức làm việc",
        color="Hình thức làm việc",
    )
    st.plotly_chart(fig, use_container_width=True)
...
with col6:
    ...
    # Biểu đồ donut
    df_experience = (
        df_filter["Yêu cầu kinh nghiệm"].value_counts().reset_index()
    )
    df_experience.columns = ["Yêu cầu kinh nghiệm", "Số lượng công việc"]
    fig = px.pie(
        df_experience,
        values="Số lượng công việc",
        names="Yêu cầu kinh nghiệm",
        hole=0.3,
    )
    st.plotly_chart(fig, use_container_width=True)
