...
with col1:
    ...
    df_highsalary = df_reshaped[df_reshaped['Lương trung bình'] >= upper_quartile]
    ...
    df_highsalary_grouped = df_highsalary.groupby('Công việc chính')['Số lượng tuyển'].sum()
    df_highsalary_grouped = pd.DataFrame(df_highsalary_grouped)
    df_highsalary = pd.merge(df_highsalary_grouped, df_copy_grouped, on='Công việc chính')
    ...
    df_highsalary.reset_index(inplace=True)
    df_highsalary.drop(columns=['index', 'Số lượng tuyển lương cao'], inplace=True)
...
with col2:
    df_grouped = df_reshaped.groupby('Công việc chính')['Lượt xem'].sum().reset_index()

    # Lấy top 10 công việc chính có nhiều lượt xem nhất
    df_top10 = df_grouped.nlargest(10, 'Lượt xem')

    # Vẽ Tree Map
    fig = px.treemap(df_top10, 
...
with col4:
    ...
    def ensure_full_datetime(date_str):
        if pd.isna(date_str):
            return date_str
        if len(date_str) == 16:  # Kiểm tra nếu độ dài chuỗi là 'YYYY-MM-DD HH:MM' hoặc 'DD/MM/YYYY HH:MM'
            return date_str + ':00'
        return date_str

    # Áp dụng hàm để đảm bảo tất cả các giá trị datetime đều có giây
    df_reshaped['Ngày cập nhật'] = df_reshaped['Ngày cập nhật'].apply(ensure_full_datetime)

    # Chuyển đổi cột 'Ngày cập nhật' thành kiểu datetime, cho phép suy luận định dạng
    df_reshaped['Ngày cập nhật'] = pd.to_datetime(df_reshaped['Ngày cập nhật'], infer_datetime_format=True, dayfirst=True, errors='coerce')
    ...
    # Vẽ biểu đồ đường
    fig = px.line(df_daily, x='Ngày', y='Số lượng tuyển', title='Số bài đăng theo ngày')
